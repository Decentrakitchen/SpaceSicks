...
const DataPoint: React.FC<DataPointProps> = ({ number, label, icon, description }) => {
  ...
  return (
    <motion.div
      ...
      className="group bg-black/20 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center hover:border-white/20 transition-all duration-300 min-h-[200px] flex flex-col justify-center items-center"
    >
      <div className="flex justify-center mb-4 text-blue-400">
        {icon}
      </div>
      <div className="group-hover:text-blue-300 transition-colors duration-300 text-white text-4xl font-bold mb-3">
        {number}
      </div>
      <div className="group-hover:text-gray-200 transition-colors duration-300 text-gray-300 text-base font-semibold mb-2">
        {label}
      </div>
      {description && (
      ...
      <section className="relative min-h-screen flex items-center justify-center">
        ...
        >
          ...
          <motion.div
            ...
            className="data-grid grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mt-16 max-w-6xl mx-auto px-4"
          >
            <DataPoint
              number="5000+"
